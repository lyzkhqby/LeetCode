...
public class StrongPasswordChecker {
    public int strongPasswordChecker(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            if (s.charAt(i) <= 'z' && s.charAt(i) >= 'a') checkL = 1;
            else if (s.charAt(i) <= 'Z' && s.charAt(i) >= 'A') checkU = 1;
            else if (s.charAt(i) <= '9' && s.charAt(i) >= '0') checkD = 1;
        ...
        if (s.length() < 6) {
            ...
            if (checkLen == 1 && checkL + checkU + checkD == 3) return addLen + 1;
